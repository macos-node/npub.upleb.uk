...
function npubToHex(input: string): string | null {
  ...
  const data5: number[] = [];
  for (const c of lower.slice(sep + 1, lower.length - 6)) {
...
type LookupState = 'idle' | 'connecting' | 'loading' | 'done' | 'error';

// ── Profile skeleton ───────────────────────────────────────────────────────────
function ProfileSkeleton() {
  return (
    <div className="bg-card border border-border p-5 flex items-start gap-4 animate-pulse">
      <div className="w-12 h-12 rounded-full bg-border/40 shrink-0" />
      <div className="flex-1 space-y-2 pt-1">
        <div className="h-3.5 w-32 bg-border/40 rounded" />
        <div className="h-2.5 w-20 bg-border/30 rounded" />
        <div className="h-2.5 w-48 bg-border/20 rounded mt-3" />
...
export default function Index() {
  ...
  // Guard: prevents ws.onerror/onclose from overwriting a completed lookup
  ...
  const doLookup = useCallback((hexPubkey: string) => {
    if (wsRef.current) {
      wsRef.current.close();
      wsRef.current = null;
    }
    ...
    let profileEose = false;
    let notesEose = false;
    const collectedNotes: NostrNote[] = [];
    ...
    const connectTimeout = setTimeout(() => {
      if (ws.readyState !== WebSocket.OPEN) {
        ...
        if (!doneRef.current) {
          setRelayError(true);
          setRelayDetail('Timed out connecting to relay.fizx.uk');
    ...
    ws.onopen = () => {
      ...
      const latency = Date.now() - connectAt;
      setRelayDetail(`Connected in ${latency}ms`);
      setLookupState('loading');
      setNotesLoading(true);
      ws.send(JSON.stringify(['REQ', 'profile', { kinds: [0], authors: [hexPubkey], limit: 1 }]));
      ws.send(JSON.stringify(['REQ', 'notes',   { kinds: [1], authors: [hexPubkey], limit: 1 }]));
    };

    // Only fires on connection failure — guard against overwriting done state
    ws.onerror = () => {
      clearTimeout(connectTimeout);
      if (!doneRef.current) {
        setRelayError(true);
        setRelayDetail('WebSocket error — relay may be offline');
        setLookupState('error');
      }
    };

    // Relay closing the connection is normal after EOSE — do NOT treat as error if done
    ws.onclose = () => {
      clearTimeout(connectTimeout);
      if (!doneRef.current && lookupState !== 'idle') {
        // Only mark error if we hadn't finished yet and didn't get far
        if (!profileEose && !notesEose) {
          setRelayError(true);
          setRelayDetail('Relay closed connection before returning results');
          setLookupState('error');
        }
      }
    };
        ...
        if (msg[0] === 'EVENT') {
          ...
          if (event.kind === 0) {
            try {
              ...
              setProfile(parsed);
              // Show profile section immediately — don't wait for notes EOSE
              if (lookupState !== 'done') setLookupState('loading');
            } catch {}
          }

          if (event.kind === 1) {
            collectedNotes.push({ id: event.id, content: event.content, created_at: event.created_at });
            setNotes([...collectedNotes].sort((a, b) => b.created_at - a.created_at));
          }
        }

        if (msg[0] === 'EOSE' && msg[1] === 'profile') {
          profileEose = true;
          ws.send(JSON.stringify(['CLOSE', 'profile']));
        }

        if (msg[0] === 'EOSE' && msg[1] === 'notes') {
          notesEose = true;
          // Mark done BEFORE closing so onerror/onclose guards work
          doneRef.current = true;
          const sorted = [...collectedNotes].sort((a, b) => b.created_at - a.created_at);
          setNotes(sorted);
          setNotesLoading(false);
          if (sorted.length === 0 && !profile) setNoEvents(true);
          setLookupState('done');
          ws.send(JSON.stringify(['CLOSE', 'notes']));
          // Close cleanly — relay will likely close it anyway, this prevents stale sockets
          setTimeout(() => { try { ws.close(); } catch {} }, 200);
        }
      } catch {}
    };

    // Fallback: wrap up if EOSE never arrives after 15s
    const eoseFallback = setTimeout(() => {
      if (!doneRef.current) {
        doneRef.current = true;
        const sorted = [...collectedNotes].sort((a, b) => b.created_at - a.created_at);
        setNotes(sorted);
        setNotesLoading(false);
        if (sorted.length === 0) setNoEvents(true);
        setLookupState('done');
        try { ws.close(); } catch {}
      }
    }, 15000);

    ws.addEventListener('close', () => clearTimeout(eoseFallback));
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setParseError(false);
    const hex = npubToHex(query);
    if (!hex) { setParseError(true); return; }
    doLookup(hex);
  };

  // Show results section as soon as we have anything — don't wait for full 'done'
  const hasAnyData = profile !== null || notes.length > 0;
  const isLoading = lookupState === 'connecting' || lookupState === 'loading';
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <nav className="border-b border-border px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center gap-4">
          ...
          <a href="https://github.com/macos-node/npub.upleb.uk" target="_blank" rel="noopener noreferrer" title="Source on GitHub" aria-label="Source on GitHub" className="shrink-0 text-muted-foreground/60 hover:text-primary transition-colors">
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-6 py-10 space-y-8">
        ...
        {/* Header — hero card (title + subtitle + relay stats + Shaka) */}
        <div>
          <div className="bg-card border border-border px-4 py-3 mb-4 min-h-[110px] flex items-start gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-start gap-1.5">
                <AnimatedTitle accent="npub" rest="" from="#FF7849" to="#FFB347" suffixRgba="rgba(255,120,73,0.2)" fontSize="clamp(28px, 5vw, 40px)" />
                <span className="mt-2.5 inline-block rounded-full border border-accent text-accent font-mono text-[10px] px-2 py-0.5 shrink-0">lookup</span>
              </div>
            </div>
            <div className="hidden sm:block shrink-0">
              <RelayStats
                urls={['wss://relay.fizx.uk', 'wss://git.upleb.uk']}
        ...
        <section>
          <div className="flex items-center gap-3 mb-3">
            ...
            <h2 className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Search</h2>
          </div>
          <form onSubmit={handleSubmit} className="flex gap-2">
            <div className="flex-1 flex items-center border border-border bg-card px-3 gap-2 focus-within:border-primary/60 transition-colors">
              ...
              <input
                ...
                className="flex-1 bg-transparent py-2.5 text-sm font-mono text-foreground placeholder:text-muted-foreground/40 outline-none"
              />
            </div>
            <button
              type="submit"
              disabled={!query.trim()}
              className="font-mono text-xs px-4 py-2 border border-primary/50 text-primary hover:bg-primary/10 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
            >
              {isLoading ? 'restart →' : 'lookup →'}
            </button>
          </form>
          {parseError && <p className="text-[10px] font-mono text-red-400/80 mt-2">Invalid npub or pubkey format</p>}

          {/* Relay status line */}
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            <p className="text-[10px] font-mono text-muted-foreground/40">
              Queries{' '}
              <a href="https://relay.fizx.uk" className="hover:text-primary transition-colors">relay.fizx.uk</a>
              {' '}for kind:0 and kind:1 events
            </p>
            {relayDetail && (
              <span className={`text-[10px] font-mono ${relayError ? 'text-red-400/60' : 'text-emerald-400/50'}`}>
                · {relayDetail}
              </span>
            )}
            {isLoading && (
              <span className="text-[10px] font-mono text-primary/40 animate-pulse">· querying…</span>
            )}
          </div>
        </section>

        {/* Error state — only when we have nothing at all */}
        {lookupState === 'error' && !hasAnyData && (
          <section>
            <h2 className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground mb-3">Profile</h2>
            <div className="bg-card border border-border p-6 space-y-3">
              <p className="text-xs font-mono text-red-400/80">{relayDetail || 'Could not connect to relay.fizx.uk'}</p>
              <button
                onClick={() => { const hex = npubToHex(query); if (hex) doLookup(hex); }}
                className="font-mono text-[10px] px-3 py-1.5 border border-border text-muted-foreground hover:text-primary hover:border-primary/30 transition-colors"
              >
                retry
              </button>
            </div>
        ...
        {/* Results — profile + most recent post, attached as one card */}
        {(hasAnyData || isLoading) && (
          <section>
            <div className="flex items-center gap-2 mb-3">
              <h2 className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground">Profile</h2>
              {isLoading && !profile && (
                <span className="flex gap-[3px]">
                  {[0,1,2].map(i => <span key={i} className="w-1 h-1 rounded-full bg-primary/50 animate-pulse" style={{ animationDelay: `${i * 150}ms` }} />)}
                </span>
              )}
            </div>
            ...
            {profile ? (
              <div className="bg-card border border-border p-5 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <p className="font-mono font-bold text-sm text-foreground truncate">
                    {profile.display_name ?? profile.name ?? <span className="text-muted-foreground/40">unnamed</span>}
                  </p>
                  {profile.name && profile.display_name && profile.name !== profile.display_name && (
                    <p className="font-mono text-[10px] text-muted-foreground/50">@{profile.name}</p>
                  )}
                  {profile.nip05 && (
                    <p className="text-[10px] font-mono text-muted-foreground/60 truncate">✓ {profile.nip05}</p>
                  )}
                  {profile.about && (
                    <p className="text-xs text-muted-foreground mt-2 leading-relaxed">{profile.about}</p>
                  )}
                  {profile.lud16 && (
                    <p className="text-[10px] font-mono text-muted-foreground/60 mt-1">⚡ {profile.lud16}</p>
                  )}
                </div>
              </div>
            ) : isLoading ? (
              ...
              <div className="bg-card border border-border p-5">
                <p className="text-xs font-mono text-muted-foreground/40">No kind:0 profile found on relay.fizx.uk for this pubkey</p>
              </div>
            )}

            {/* Most recent post — attached directly under the profile card */}
            {notes.length > 0 ? (
              <div className="bg-card border-x border-b border-border px-5 py-4">
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="text-[10px] font-mono uppercase tracking-widest text-muted-foreground/50">Most recent</span>
                  <span className="text-[10px] font-mono text-muted-foreground/40">· {relativeTime(notes[0].created_at)}</span>
                </div>
                <p className="text-xs text-foreground leading-relaxed break-words whitespace-pre-wrap">
                  {notes[0].content.length > 280 ? notes[0].content.slice(0, 280) + '…' : notes[0].content}
                </p>
              </div>
            ) : notesLoading ? (
              <div className="bg-card border-x border-b border-border px-5 py-4 animate-pulse">
                <div className="h-2.5 bg-border/30 rounded w-1/4 mb-2" />
                <div className="h-2.5 bg-border/20 rounded w-3/4" />
              </div>
            ) : (profile || noEvents) ? (
              <div className="bg-card border-x border-b border-border px-5 py-3">
                <p className="text-[10px] font-mono text-muted-foreground/40">No kind:1 notes found on relay.fizx.uk</p>
              </div>
            ) : null}
          </section>
        )}
        ...
        {lookupState === 'idle' && (
          <section>
            ...
            <div className="border border-dashed border-border/50 p-10 text-center">
              <p className="text-xs font-mono text-muted-foreground/40">Enter an npub above to view profile and events</p>
            </div>
          </section>
        )}
